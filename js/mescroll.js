...
(function (name, definition) {
  ...
  var MeScroll = function (mescrollId, options) {
    ...
    me.initialValue = this._getValue(me.options.initialValue, 0);
    this.fixed = this._getValue(me.options.fixed, false);
    this.sensitivity = this._getValue(me.options.sensitivity, 1);
    this.moveFactor = this._getValue(me.options.moveFactor, 1);
    this.factor = this._getValue(me.options.factor, 1);
    this.outFactor = this._getValue(me.options.outFactor, 0.3);
    this.deceleration = 0.0006;
    this.lockDirection = this._getValue(me.options.lockDirection, true);
  ...
  MeScroll.prototype.extendDownScroll = function (optDown) {
    ...
    MeScroll.extend(optDown, {
      ...
      onMoving: function (mescroll, downHight) {
        console.log(downHight+'down')
        var rate = downHight / mescroll.optDown.offset; // 下拉区域当前高度与指定距离的比值
         //mescroll.downwarp.style.height = downHight + 'px';
 ...
 MeScroll.prototype._getValue=function (obj, defaultValue) {
            return obj === void 0 ? defaultValue : obj;
        }
  MeScroll.prototype._to=function (value, time, ease) {

            if (this.fixed) return;
           // var el = this.target,
                //property = this.property;
            var current = this.initialValue
            
            if(this._vMove){
                
                var dv = value - this.initialValue
                var startY=this.initialValue
                ...
                var toTick = function () {
                    ...
                    if (dt >= time) {
                       ...
                       self.downHight=self.initialValue
                        return;
                    }
                    self.initialValue = dv * ease(dt / time) + startY;
                   self.downHight=self.initialValue
                    self.optDown.onMoving(self,self.initialValue)
                   //self.downwarp.style.height = self.initialValue + 'px';
  ...
  MeScroll.prototype.initDownScroll = function () {
    ...
    me.touchmoveEvent = function (e) {
      ...
      if (me._firstTouchMove && me.lockDirection) {
                    var dDis = Math.abs(curPoint.x - me.startPoint.x) - Math.abs(curPoint.y - me.startPoint.y);
                    if (dDis > 0 ) {
                        //横向运动
                        me._hMove=true;//add
                        me._vMove=false;//add
        ...
        if (scrollTop <= 0) {
          ...
          if (me.optDown.use && !me.inTouchend && !me.isDownScrolling && !me.optDown.isLock && (!me.isUpScrolling || (me.isUpScrolling && me.optUp.isBoth))) {
           ...
           /* var x = Math.abs(me.lastPoint.x - curPoint.x);
            ...
            me.initialValue=me.downHight
            
            // 实时更新下拉区域高度
            
            me.optDown.onMoving(me, me.downHight); // 下拉过程中的回调,一直在执行
          }
        }
