...
(function () {
    ...
    AlloyTouch.prototype = {
        ...
        _start: function (evt) {
            ...
            this.touchStart.call(this, evt, this.initialValue);
            cancelAnimationFrame(this.tickID);
            this._calculateIndex();
        ...
        _move: function (evt) {
            if (this.isTouchStart) {
                ...
                if(!this._preventMove) {
                    ...
                    if (this.hasMax && this.initialValue > this.max && d > 0) {
                        f = this.outFactor;
                    } else if (this.hasMin && this.initialValue < this.min && d < 0) {
                        f = this.outFactor;
                    }
                    d *= f;
                    this.preX = currentX;
                    ...
                    this.change.call(this, this.initialValue);
                    var timestamp = new Date().getTime();
                    if (timestamp - this.startTime > 300) {
                        this.startTime = timestamp;
                        this.start = this.vertical ? this.preY : this.preX;
                    }
                    this.touchMove.call(this, evt, this.initialValue);
                }
                ...
                if (len === 1) {
                    ...
                    this.pressMove.call(this, evt, this.initialValue);
                }
                this.x2 = currentX;
                this.y2 = currentY;
            }
        ...
        _calculateIndex: function () {
            if (this.hasMax && this.hasMin) {
                this.currentPage = Math.round((this.max - this.initialValue) / this.step);
            }
        },
        _end: function (evt) {
            if (this.isTouchStart) {
                ...
                if (this.hasMax && current > this.max) {
                    ...
                    if (dt < 300) {
                        ...
                        self._to(Math.round(destination), duration, ease, self.change, function (value) {
                            if (self.hasMax && self.initialValue > self.max) {

                                cancelAnimationFrame(self.tickID);
                                self._to(self.max, 600, ease, self.change, self.animationEnd);

                            } else if (self.hasMin && self.initialValue < self.min) {
        ...
        _to: function (value, time, ease, onChange, onEnd) {
           ...
           // var el = this.target,
            ...
            var toTick = function () {
                ...
                self.initialValue = dv * ease(dt / time) + current;
                //console.log(self.initialValue+'ddddddddd')
                self.tickID = requestAnimationFrame(toTick);
                //cancelAnimationFrame必须在 tickID = requestAnimationFrame(toTick);的后面
                onChange && onChange.call(self,  self.initialValue);
            };
            toTick();
        },
